...
7. ____ **Initialize the Telescope.** Launch *SiTech.exe* and click *SkyView*. Turn on the telescope with the key on the RA skirt. After several seconds, the F1 will detect motor power and the blinking lights will change to solid red. Do not proceed to the next step until all LED lights in the telescope pier are green.

8. ____ **Update the telescopes coordinates.**  If the telescope has been properly parked, it is pointed at zenith. If you're not sure, use the bubble level and the hand paddle to bring to telescope to zenith. Resist the urge to make small adjustments--if the telescope appears to be very near zenith, it was almost certainly properly parked! ONLY correct for gross offsets, as you will more finely tune the pointing at a later stage. After you have confirmed that the telescope is pointed at zenith, find zenith in SkyView, zoom-in, right-click, choose "Goto or Sync", Sync, and OK to update the telescope's  coordinates. The telescope will start tracking with the RA set to the LST and Dec set to the latitude of MRO: 46:57:04.2.
